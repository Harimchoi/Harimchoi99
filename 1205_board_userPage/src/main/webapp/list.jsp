...
<%	
	...
	//사용자가 원하는 1페이지당 레코드 개수가져오기
	int len; //1페이지당 레코드 개수 담을 변수
	...
	int index = (pager-1)*len;  //select sql문의 limit의 첫번째 index값 구하기 

	//2. 쿼리생성
	String sql = "select * from board order by id desc limit "+index+", "+len;
...
<!-- 사용자가 페이지 수 결정하는 셀렉트 시작 -->
	<tr>
		<td colspan="5" class="pgTbl">페이지수
		<select id ="len" onChange="move(this.value)">
...
<!-- 사용자가 페이지 수 결정하는 셀렉트 끝 -->	
	...
	%>
	<tr onMouseover="over(this)" onMouseout="out(this)" class="<%if(line%2==0){out.print("bg");}%>" onclick="location='readnum.jsp?id=<%=rs.getString("id")%>&pager=<%=pager%>&len=<%=len%>'" style="cursor:pointer">
	...
	%>
	<tr>
		<td colspan="5"><input type="button" value="글쓰기" onclick="location.href='write.jsp?&len=<%=len%>'"></td>
	</tr>
	<!-- 페이지 시작 ********************* -->
	<tr>
	<%
	...
	%>
		<td colspan="5">
		<!-- 무조건 1페이지로 이동  시작 -->
		<span onClick="location='list.jsp?pager=1&len=<%=len%>'" class="page link">↖</span>
		<!-- 무조건 1페이지로 이동  끝 -->
		<!-- 10페이지 단위로 이전으로 이동하기 시작 -->
		<%
			if(pstart != 1) {
		%>
			<span onClick="location='list.jsp?pager=<%=pstart-1%>&len=<%=len%>'" class="page link">◀</span>
		<%
			}
			else
			{
		...
		<%
		 ...
		 String sql_Rcnt = "select ceil(count(*)/"+len+") as cnt from board";
		...
		<span onclick="location='list.jsp?pager=<%=i%>&len=<%=len%>'" class="page link" <%=str%>><%=i%></span>
			<%
				}
				while(cntPg<10){
			%>
		...
		<%
			if(cnt != pend ) {
		%>
			<span onClick="location='list.jsp?pager=<%=pend+1%>&len=<%=len%>'" class="page link">▶</span>
		<%
			}
			else
			{
		...
		<!-- 무조건 마지막페이지로 이동  시작 -->
		<span onClick="location='list.jsp?pager=<%=cnt%>&len=<%=len%>'" class="page link">↗</span>
		<!-- 무조건 마지막페이지로 이동  끝 -->
		</td>
	</tr>
	<!-- 페이지 끝 ********************* -->
