...
public class Member {
  ...
  //int 생년월일;
	...
	// 2.전체 필드를 갖는 생성자
	// 마우스 오른쪽 클릭 => source => 생성자 혹은 메소드 등 자동완성
	...
	public void 회원가입() {
		...
		System.out.println("아이디 : ");					String 아이디 = Day07_2_Start.scanner.next();
		//아이디 중복체크
		//int 중복체크 = 0;
		...
		System.out.println("비밀번호 : "); 				String 비밀번호 = Day07_2_Start.scanner.next();
		System.out.println("이름 : "); 					String 이름 = Day07_2_Start.scanner.next();
		System.out.println("성별 : 남자:1 여자:2 미설정:3"); int 성별 = Day07_2_Start.scanner.nextInt();
		System.out.println("휴대전화 : "); 				int 휴대전화 = Day07_2_Start.scanner.nextInt();
		...
		try {
			...
			for(Member temp : Day07_2_Start.members) {
				...
				+temp.이름+","+temp.성별+","+temp.이메일+","+temp.포인트+","
						+temp.휴대전화+","+temp.등급+"\n";
