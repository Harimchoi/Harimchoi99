...
public class Day07_1 {
	...
	//1.변수 : 자료형[메모리크기가 정해진]을 이용한 메모리 할당
		...
		int 변수명 =30;
		//Integer 변수명2 = new Integer();
	//2.객체 : 클래스[설계도]를 이용한 메모리 할당
		//메모리의 크기가 정해져 있지 않기 때문에 new 연산자 x
		//Scanner 객체 = new Scanner();
	//메모리 관리 방법
		//1. 배열 : 동일한 자료형/클래스의 여러개 메모리 선언/관리
		int[]배열 = new int[30];
		//2. 리스트 : 동일한 자료형 / 클래스의 여러개 메모리를 저장할 수 있는 객체(컬렉션프레임워크)(가변)
			ArrayList<Integer> 리스트 = new ArrayList<>(); //<제네릭>
	...
	public static void main(String[] args) {
		...
		temp1.회원정보();
		System.out.println("점수"+temp1.학생점수(20));
		...
		try {//try 안에서 예외발생시 => catch 이동
				//try 안에서 예외가 없을 경우는 그대로 실행
			FileOutputStream fileOutputStream = new FileOutputStream("c:/java/test.txt");
			
			String 문자열 = "java입니다.";
			fileOutputStream.write(문자열.getBytes()); //문자열이 아닌경우
				//write(바이트) : 출력[내보내기] : 
			...
			try {
				...
				//스트림의 이동단위 바이트
				byte[] 바이트 = new byte[1024];	//1024바이트 => 1kbyte
